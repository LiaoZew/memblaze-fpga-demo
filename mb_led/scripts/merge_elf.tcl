...
# (needs dist/mb_led.elf):
#   vivado -mode batch -source mb_led/scripts/merge_elf.tcl
#
# Publishes (overwrites):
#   dist/mb_led.bit  - bitstream with the program embedded (JTAG program
#                      and the LEDs run immediately - no separate download)
#   dist/mb_led.bin  - SPIx4 flash image with the program embedded
#==============================================================================

set script_dir [file dirname [info script]]
set root       [file normalize [file join $script_dir ..]]
set repo_root  [file normalize [file join $script_dir ../..]]
set impl_dir   [file join $root projects mb_led.runs impl_1]
...
set elf_src [file join $dist_dir mb_led.elf]
...
set merged_bit [file join $impl_dir mb_led_merged.bit]
puts "updatemem: bit=$bit_src mmi=$mmi_src elf=$elf_src"
set cmd "$um -meminfo $mmi_src -bit $bit_src -data $elf_src -proc system_i/microblaze_0 -force -out $merged_bit"
puts "RUN: $cmd"
...
file copy -force $merged_bit [file join $dist_dir mb_led.bit]
puts "PUBLISH: [file join $dist_dir mb_led.bit]  (ELF embedded)"
...
if {$dcp_src ne ""} {
    ...
    write_cfgmem -force -format bin -interface SPIx4 \
                 ...
                 -file [file join $impl_dir mb_led.bin]
    file copy -force [file join $impl_dir mb_led.bin] [file join $dist_dir mb_led.bin]
    puts "PUBLISH: [file join $dist_dir mb_led.bin]  (ELF embedded)"
    close_design
} else {
    puts "WARNING: no routed checkpoint, .bin not regenerated"
}
puts "=== DONE. dist/mb_led.bit/.bin now include the MicroBlaze program. ==="
puts "=== Burn: vivado -mode batch -source dist/program_flash.tcl -tclargs mb_led.bin ==="